...
| Envelope | 3 inconsistent variants (`{msg,…}`, `{status,message}`, `{ok:false,…}`) | bare DTO + `{ error: { code, message, fieldErrors, requestId, status } }` | Contract fixed at gateway; no per-app variants |
