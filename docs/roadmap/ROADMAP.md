...
| 4 | Guest checkout & tickets — discovery/directory, pricing, Razorpay checkout, promo codes, entitlement/QR, wallet | **DOMAIN DONE, HTTP SURFACE MISSING** — domain models, application services (`CheckoutService` wired in `v2-services.ts`), adapters and contracts are committed and tested, but **no route file for checkout/payments/orders/tickets/wallet/webhooks/public was ever committed** (`a1bd2e7` "Phase 4 complete" touched only `v2-services.ts` under `apps/`; the route files lived in the working tree destroyed 2026-08-28 and were not in the `7e2d6c9` recovery). The "302 tests pass" line was measured on that tree, not a commit. **Rebuild = route-layer wiring + route tests + move `razorpay-adapter.ts` out of core.** See `C1RCLE-FRONTEND/docs/superpowers/SPRINT-2026-08-31.md` §P1-A. | [phase-04-guest-checkout-tickets.md](phase-04-guest-checkout-tickets.md) |
| 5 | Door / Scanner / Cover-wallet — entitlement scan + magic-ticket QR, walk-in/dine-in, cover-charge wallet | **substantially done** (2026-08-21) — domain + memory/Firestore adapters + HTTP routes wired to real services for scanner/session/offline-sync/magic-QR (`door/scanner-routes.ts`), door-sale (`door/door-sale-routes.ts`), cover-wallet (`door/cover-wallet-routes.ts`). Core `pnpm test` 232/232 green (the old "231/232, 1 known failure" was `compare-and-set.test.ts` on the pre-recovery tree — green since `7e2d6c9`). Still 501, by design: live door stats + WebSocket (needs `@fastify/websocket` + aggregation design), `/door/override` (FSM has no override transition), `/door/offline-manifest` (nothing signs one), cover-wallet freeze/unfreeze (no service method). **Missing:** domain-model unit tests for the 5 Phase-5 models. See `docs/PHASE_5_HTTP_WIRING_PLAN.md`. | [phase-05-door-scanner-cover-wallet.md](phase-05-door-scanner-cover-wallet.md) |
