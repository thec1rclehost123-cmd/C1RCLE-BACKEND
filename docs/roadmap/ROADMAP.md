...
| 5 | Door / Scanner / Cover-wallet — entitlement scan + magic-ticket QR, walk-in/dine-in, cover-charge wallet | **substantially done** (2026-08-21, verified) — domain + memory/Firestore adapters + HTTP routes wired to real services for scanner/session/offline-sync/magic-QR (`door/scanner-routes.ts`), door-sale (`door/door-sale-routes.ts`), cover-wallet (`door/cover-wallet-routes.ts`); `pnpm --filter @c1rcle/core build` clean, gateway 122/122 tests pass, core 231/232 (1 pre-existing unrelated failure). Still BLOCKED/501: live door stats + its WebSocket (needs `@fastify/websocket` + an aggregation design), `/door/override` (FSM has no override transition), `/door/offline-manifest` (nothing signs one), cover-wallet freeze/unfreeze (no service method). See `docs/PHASE_5_HTTP_WIRING_PLAN.md` for exact verification commands and open gaps. | [phase-05-door-scanner-cover-wallet.md](phase-05-door-scanner-cover-wallet.md) |
