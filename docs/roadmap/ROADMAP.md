...
| 1 | Partner dashboards — Host/Venue/Promoter overview+events+finance+analytics, RBAC/tab-visibility, partnerships | **done** (2026-08-13); finance/analytics shipped with Phase 6 (2026-09-08); `venueShareRate` field still open (see Phase 6 note) | [phase-01-partner-dashboards.md](phase-01-partner-dashboards.md) |
...
| 4 | Guest checkout & tickets — discovery/directory, pricing, Razorpay checkout, promo codes, entitlement/QR, wallet | **done** (verified 2026-09-07) — route files for checkout, payments, webhooks, orders, tickets, wallet, public all committed under `apps/api-gateway/src/routes/v2/{checkout,orders,tickets,wallet,public}/` with route tests alongside each. | [phase-04-guest-checkout-tickets.md](phase-04-guest-checkout-tickets.md) |
| 5 | Door / Scanner / Cover-wallet — entitlement scan + magic-ticket QR, walk-in/dine-in, cover-charge wallet | **done** (verified 2026-09-07) — `/door/override`, `/door/stats`, cover-wallet freeze/unfreeze all implemented and tested (see commits `0342d80`, `53727c8`, `2ec1e61`); the 5 Phase-5 domain-model unit test files landed in `3f48069`. Only two honest, by-design 501s remain: `/door/offline-manifest` and `/door/stats/ws` (`phase5-routes.ts:59`) — neither has a signing/WS service edge to call, documented as such at the call sites. | [phase-05-door-scanner-cover-wallet.md](phase-05-door-scanner-cover-wallet.md) |
| 6 | Finance / Ledger / Payouts — settlement engine, bank accounts, disputes, T+3 batch | **done** (verified 2026-09-08) — ledger, balances, payouts, bank accounts, disputes, checkout-webhook integration, and promoter leaderboard all built + tested. Known gap: `venueShareRate` settles to 0 (no persisted venue-revenue-share field exists yet). See phase file Session Log. | [phase-06-finance-ledger-payouts.md](phase-06-finance-ledger-payouts.md) |
| 7 | Admin console backend — tiered-authority approvals, propose→resolve, audit log | **done** (A/B/D 2026-09-16) — 15 admin route files + 14 route test files under `routes/v2/admin/` (onboarding-review, directory, refunds, payouts, disputes, orders, tickets, promotions, promoters, analytics, venue/org/event/user action desks); Phase C (support/safety/content) paused & invite-by-email deferred — track live in [ADMIN-DASHBOARD-GAPS.md](ADMIN-DASHBOARD-GAPS.md) & [phase-07-admin-console.md](phase-07-admin-console.md) | [phase-07-admin-console.md](phase-07-admin-console.md) |
