...
| IP/UA capture in audit records | ✅ **IMPLEMENTED 2026-09-18** — `AuditInput`/`AdminAuditRecord` gained optional `ipAddress`/`userAgent`, persisted by `AdminAuthorityService.record`. Event-action routes (`pause`/`resume`/`force-complete`) capture Fastify `request.ip` + `user-agent` and thread them through. ⚠️ Remaining sweep: the other ~22 call sites across venue/org/user/payout/refund/promoter desks still omit meta — same `requestMeta` pattern to apply desk-by-desk. | Phase D scope note |
