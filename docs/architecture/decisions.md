...
## D-030 · Partner-chain audit closes: N+1 name resolution, block TOCTOU, poster ACL, and the `maxPerUser` knob (2026-09-21)

- **Date / Status:** 2026-09-21 · **enforced** (delivered as review fixes on `feat/partner-create-event-flow-v2`, back-ported to `feat/calendar-changes` and `fix/partner-connection`; regression tests added at the gateway level).
- **Context:** Pre-merge audit of the intern partner branches found four gaps between what the wire advertised and what the code enforced, each with a real failure mode in production:
  1. `listWithNames` (partnerships + promoter connections) fetched counterparty rows one-at-a-time — up to 300 Firestore reads per 100-row page.
  2. Venue slot `block()` did a read-check-write with no atomicity — two concurrent blocks for the same minutes could both pass the overlap guard (double-booking).
  3. `issuePosterUploadUrl` returned a `toPublicUrl()` for an object the signed PUT uploaded as **private** — posters 403'd on the guest surface; naive fixes (bucket-wide public-read) would have exposed KYC.
  4. Tier `maxPerUser` was accepted by the wire schema and returned by the DTO but never enforced — a security-sounding knob that silently did nothing.
- **Choice — four resolutions, each in the adapter/service layer per D-005/D-015:**
  1. **Batched name resolution.** `OrganizationRepository.getByIds` / `VenueRepository.getByIds` added to the port (Firestore: chunked `getAll` at 30 refs/call, deduped; memory: map lookups). Both `listWithNames` implementations now do a constant three batched reads per page regardless of page size. Missing rows resolve to `null`-safe fields, never an error — batch lookup is name resolution, not existence checks.
  2. **Atomic block creation.** `VenueSlotRepository.createBlockIfFree(block)` owns the read-check-write in one storage transaction: the Firestore adapter runs the overlap query and the insert inside `db.runTransaction` (reusing domain `assertSlotRangeFree`, so the failure shape is unchanged — 400, never a corrupted timeline); the memory adapter performs the guard and the push in one synchronous turn. `block()` now delegates entirely to it. All venue-slot reads (`listSlots`, `listOverlappingSlots`, the in-transaction query) are additionally capped at `MAX_SLOTS_READ = 2000` so a venue that accumulates thousands of `cancelled` tombstones never turns a calendar read into an unbounded scan.
  3. **Object-level poster ACL.** `UploadUrlRequest` gains `visibility: 'private' | 'public'` (default private). A `'public'` request signs `x-goog-acl: public-read` into the v4 PUT (`extensionHeaders` **and** the returned PUT headers), so the browser's upload sets **that object's** ACL — the bucket, and every KYC/private object, stay credential-only. `issuePosterUploadUrl` opts in; the onboarding KYC flow stays private-by-default. `EchoObjectStorage` mirrors the header for memory-driver parity.
  4. **`maxPerUser` enforced at hold time** *(ships with `feat/partner-create-event-flow-v2` only — the tier-DTO expansion it depends on is not on this branch).* `createHold` loads each line's tier and, when a stable buyer id exists (`actor.userId` — guests on this platform carry an `x-user-id`), counts what the user already holds (live cart reservations: `CartReservationRepository.countActiveQuantity`) and owns (paid orders only: `OrderRepository.countPaidQuantityByUserAndEvent`) for that `(user, event, tier)`, rejecting when `held + bought + requested > maxPerUser`. Both counts are single-field `userId` equality queries filtered in application code — no new composite indexes. A guest with **no** identity at all falls through (documented in code): there is no stable key to count against, and v1 never enforced a per-user tier cap either. Resolutions 1–3 apply on this branch; resolution 4 lands when the v2 branch merges.
- **Why:** Each failure broke a documented invariant (bounded reads B-x, no lost updates D-015, no PII exposure D-018, wiring honest to its schema). The fixes preserve the frozen wire contract — every change is additive (new optional fields, new repo methods) and the DTO shapes are byte-identical, so the frontend mirror only needs its standard `scripts/export-contracts.mjs` regeneration, not a contract break.